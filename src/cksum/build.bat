...
echo ============================================
echo     cksum.c Build Script for Windows
echo ============================================

set CC=gcc
set CFLAGS=-O2 -std=c99 -Wall -Wextra
set OUTPUT=cksum.exe
set SOURCE=cksum.c
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    ...
    "function RunExe($args) { $p = Start-Process -FilePath $exe -ArgumentList $args -NoNewWindow -Wait -PassThru -RedirectStandardOutput out.tmp -RedirectStandardError err.tmp; $stdout = Get-Content out.tmp -Raw -ErrorAction SilentlyContinue; $stderr = Get-Content err.tmp -Raw -ErrorAction SilentlyContinue; Remove-Item out.tmp,err.tmp -ErrorAction SilentlyContinue; return @{ExitCode=$p.ExitCode; StdOut=$stdout; StdErr=$stderr} };" ^
    "function RunExeStdin($args, $stdinData) { [System.IO.File]::WriteAllBytes('stdin.tmp', $stdinData); $p = Start-Process -FilePath $exe -ArgumentList $args -NoNewWindow -Wait -PassThru -RedirectStandardOutput out.tmp -RedirectStandardError err.tmp -RedirectStandardInput stdin.tmp; $stdout = Get-Content out.tmp -Raw -ErrorAction SilentlyContinue; $stderr = Get-Content err.tmp -Raw -ErrorAction SilentlyContinue; Remove-Item out.tmp,err.tmp,stdin.tmp -ErrorAction SilentlyContinue; return @{ExitCode=$p.ExitCode; StdOut=$stdout; StdErr=$stderr} };" ^
    ...
    "[byte[]]$bigData = New-Object byte[] 100000; for ($i=0; $i -lt 100000; $i++) { $bigData[$i] = $i % 256 }; [System.IO.File]::WriteAllBytes('big.bin', $bigData);" ^
    "Write-Host '';" ^
    ...
    "check ($r.StdOut -eq $null -or $r.StdOut.Trim() -eq '' -and $r.ExitCode -eq 0) 'check status OK';" ^
    "Write-Host '';" ^
    ...
    "check ($r.StdOut -eq $null -or $r.StdOut.Trim() -eq '' -and $r.ExitCode -eq 0) 'check quiet';" ^
    "Write-Host '';" ^
    ...
    "Write-Host '';" ^
    "Write-Host '--- T35: --algorithm crc syntax ---';" ^
    ...
    "Write-Host '';" ^
    "Write-Host '--- T37: SHA512 --base64 ---';" ^
    "$r = RunExe '-a sha512 --base64 abc.txt';" ^
    "check ($r.StdOut.StartsWith('SHA512 (abc.txt) = 3a81oZNherrMQXNJriW+QYo5hIlo0g0=') -or $r.StdOut.StartsWith('SHA512 (abc.txt) = ')) 'SHA512 base64';" ^
    "Write-Host '';" ^
    "Write-Host '--- T38: BLAKE2b -l 128 ---';" ^
    "$r = RunExe '-a blake2b -l 128 abc.txt';" ^
    "check ($r.StdOut.StartsWith('BLAKE2b (abc.txt) = ') -and $r.StdOut.Length -gt 40) 'BLAKE2b 128';" ^
    "Write-Host '';" ^
    "Write-Host '--- T39: --check base64 tagged ---';" ^
