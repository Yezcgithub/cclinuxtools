#!/bin/bash
# Build and test script for md5sum.c (Unix/Linux/macOS/BSD)

# NOTE: Do NOT use 'set -e' — md5sum -c returns 1 on FAILED/missing
#       files, which would abort the script on those test cases.

echo "============================================"
echo "    md5sum.c Build Script for Unix"
echo "============================================"
...
out=$(./"$OUTPUT" abc.txt)
[ "$out" = "900150983cd24fb0d6963f7d28e17f72  abc.txt" ]
check $? "abc default"
...
[ "$out" = "d41d8cd98f00b204e9800998ecf8427e  empty.txt" ]
check $? "empty default"
...
[ "$out" = "MD5 (stdin) = 900150983cd24fb0d6963f7d28e17f72" ]
check $? "tag stdin"

# ========== T07: --check untagged OK
echo ""
echo "--- T07: --check untagged OK ---"
printf '900150983cd24fb0d6963f7d28e17f72  abc.txt\n' > check_ok.txt
...
printf 'MD5 (abc.txt) = 900150983cd24fb0d6963f7d28e17f72\n' > check_tag.txt
...
printf '00000000000000000000000000000000  abc.txt\n' > check_bad.txt
...
printf '900150983cd24fb0d6963f7d28e17f72  nonexistent.txt\n' > check_missing.txt
...
code=$?
[ $code -eq 0 ] && echo "$out" | grep -q 'md5sum'
check $? "version"
