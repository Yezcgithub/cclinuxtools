# Comprehensive test suite for sha256sum.c
# Run with: powershell -NoProfile -ExecutionPolicy Bypass -File _run_tests.ps1

$ErrorActionPreference = 'Continue'
$exe = Join-Path $PSScriptRoot 'sha256sum.exe'
$PASS = 0
$FAIL = 0
...
function RunExe($argStr) {
    ...
    $stdout = ''
    $stderr = ''
    if (Test-Path out.tmp) {
        $raw = Get-Content out.tmp -Raw -ErrorAction SilentlyContinue
        if ($raw) { $stdout = $raw }
    }
    if (Test-Path err.tmp) {
        $raw = Get-Content err.tmp -Raw -ErrorAction SilentlyContinue
        if ($raw) { $stderr = $raw }
    }
...
function Cleanup {
    $files = @('abc.txt', 'empty.txt', 'big.bin',
               'check_ok.txt', 'check_tag.txt', 'check_bad.txt',
               'check_missing.txt', 'check_bin.txt', 'check_warn.txt',
               'check_empty.txt', 'check_novalid.txt',
...
$r = RunExe 'abc.txt'
check ($r.StdOut.Trim() -eq "$ABC_HEX  abc.txt") 'abc default'

Write-Host ''
Write-Host '--- T02: empty file ---'
$r = RunExe 'empty.txt'
check ($r.StdOut.Trim() -eq "$EMPTY_HEX  empty.txt") 'empty default'

Write-Host ''
Write-Host '--- T03: stdin ---'
$r = RunExeStdin '-' $abcBytes
check ($r.StdOut.Trim() -eq "$ABC_HEX  -") 'stdin'

Write-Host ''
Write-Host '--- T04: -b binary mode ---'
$r = RunExe '-b abc.txt'
check ($r.StdOut.Trim() -eq "$ABC_HEX *abc.txt") 'binary mode'

Write-Host ''
Write-Host '--- T05: --tag mode ---'
$r = RunExe '--tag abc.txt'
check ($r.StdOut.Trim() -eq "SHA256 (abc.txt) = $ABC_HEX") 'tag mode'

Write-Host ''
Write-Host '--- T06: --tag stdin ---'
$r = RunExeStdin '--tag -' $abcBytes
check ($r.StdOut.Trim() -eq "SHA256 (stdin) = $ABC_HEX") 'tag stdin'
...
[System.IO.File]::WriteAllText('check_tag.txt', "SHA256 (abc.txt) = $ABC_HEX`n")
...
$badHash = '0' * 64
[System.IO.File]::WriteAllText('check_bad.txt', "$badHash  abc.txt`n")
...
check (($r.StdOut.Trim() -eq '') -and ($r.ExitCode -eq 0)) 'check quiet OK'
...
check ($r.ExitCode -ne 0) 'missing file fails'

Write-Host ''
Write-Host '--- T15: multiple files ---'
$r = RunExe 'abc.txt empty.txt'
check (($r.StdOut -match [regex]::Escape($ABC_HEX)) -and
       ($r.StdOut -match [regex]::Escape($EMPTY_HEX))) 'multiple files'

Write-Host ''
Write-Host '--- T16: --help exits 0 ---'
$r = RunExe '--help'
check (($r.ExitCode -eq 0) -and ($r.StdOut -match 'Usage:')) 'help'

Write-Host ''
Write-Host '--- T17: --version exits 0 ---'
$r = RunExe '--version'
check (($r.ExitCode -eq 0) -and ($r.StdOut -match 'sha256sum')) 'version'

Write-Host ''
Write-Host '--- T18: big file ---'
$r = RunExe 'big.bin'
check ($r.StdOut -match 'big\.bin') 'big file exists'
...
check (($r.StdErr -match 'improperly formatted') -and ($r.ExitCode -eq 0)) 'warn bad line'
...
$r = RunExe '-t abc.txt'
check ($r.StdOut.Trim() -eq "$ABC_HEX  abc.txt") 'text mode'
...
check ($r.StdOut.Trim() -eq "SHA256 (abc.txt) = $ABC_HEX") 'tag + binary'

Write-Host ''
Write-Host '--- T27: --zero output ---'
$r = RunExe '--zero abc.txt'
check ($r.StdOut -match "`0") 'zero output'

Write-Host ''
Write-Host '--- T28: unrecognized option fails ---'
$r = RunExe '--foobar'
check (($r.ExitCode -ne 0) -and ($r.StdErr -match 'unrecognized')) 'unrecognized option'

Write-Host ''
Write-Host '--- T29: nonexistent file fails ---'
$r = RunExe 'nonexistent.txt'
check (($r.ExitCode -ne 0) -and ($r.StdErr -match 'No such file')) 'nonexistent file'
